...
with open('test-data-settings-64.json') as data_file:
    ...
    output_image_height = data['output_image_height']
    output_image_width = data['output_image_width']
...
for file_count in range(0, files_to_generate):
    try:
        ...
        offset_x = random.randint(0, 140)
        offset_y = random.randint(0, 140)


        # new_image = Image.new('L', (total_width, max_height)) # mode was 'RGB'
        mode = 'L'
        full_size = max(total_width + offset_x, total_width + offset_y)
        new_image = Image.new(mode=mode, size=(full_size, full_size))


        x_offset = 0
        for im in images:
            new_image.paste(im, (x_offset + offset_x, 0 + offset_y))
            x_offset += im.size[0]

        # let's resize before we go
        resize = (output_image_height, output_image_width)
        new_image.thumbnail(size=resize, resample=Image.BICUBIC)
